...
<head>
    ...
    <style>
        ...
        th, td { padding: 8px 6px; text-align: left; border-bottom: 1px solid #eee; font-size: 12px; word-break: break-all; }
        ...
        .tip { text-align: center; color: #fff; margin-top: 12px; font-size: 12px; opacity: 0.9; }
...
<body>
    ...
    <script>
        ...
        function search() {
            ...
            let h = '<div class="org-header"><h2>🏢 ' + orgName + '</h2><p>机构号：' + matchedCode + ' | 共' + arr.length + '条设备信息</p></div><table><thead><tr><th>类型</th><th>品牌</th><th>维护商</th><th>负责人</th><th>电话</th><th>备注</th></tr></thead><tbody>';
...
# 创建一个自包含的HTML（嵌入所有数据）
print('HTML已保存:', html_path)
print('\n机构号-网点名称映射示例:')
for i, (k, v) in enumerate(org_map.items()):
    if i < 10: print(f'  {k} -> {v}')
print(f'  ... 共 {len(org_map)} 条')

print('\n注意: 手机扫码需要将HTML部署到服务器才能访问')
print('如需生成在线链接，建议先将HTML文件上传到可访问的服务器')
